...
const AIChatModal = ({ isOpen, onClose, pageContext, sessionId, messages, setMessages, guestId, restaurantId, tableNumber, isFirstLaunch = false, prewarm = false, onExpandDish, onCartAdd, onShowCart, onHideCart }) => {
  ...
  // Авто-приветствие текстового чата УБРАНО намеренно: раньше при каждом
  // открытии текстового режима сюда слался маркер "ПРИВЕТСТВИЕ", и ИИ сам
  // начинал печатать (с учётом раздела меню). Теперь ИИ молчит, пока гость
  // не напишет первым. Раздел/блюдо, из которого открыт чат, всё равно
  // учитывается — он уходит в n8n полем context с КАЖДЫМ сообщением гостя
  // (см. handleActionClick), так что при первом же вопросе ИИ его видит.
