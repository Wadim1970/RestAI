-- ============================================================
-- Фикс: sync_guest_cart падал с 42702 «column reference "session_id" is
-- ambiguous».
--
-- ПРИЧИНА: RETURNS TABLE (session_id uuid, seat_number integer) делает
-- session_id выходной ПЕРЕМЕННОЙ, а в теле остались НЕквалифицированные
-- ссылки на КОЛОНКУ session_id таблицы guest_cart_drafts (в UPDATE и в
-- подзапросе COUNT). PL/pgSQL не мог решить, колонка это или переменная,
-- и функция валилась на каждом вызове → черновики корзин не писались.
--
-- РЕШЕНИЕ: квалифицировать эти ссылки псевдонимом таблицы (gcd.). Форма
-- вызова и результата не меняется.
--
--   sudo docker exec -i supabase-db psql -U supabase_admin -d postgres \
--     -v ON_ERROR_STOP=1 < supabase/migrations/20260725020000_fix_sync_guest_cart_ambiguous.sql
-- ============================================================
...
BEGIN
  ...
  ELSE
    UPDATE public.guest_cart_drafts AS gcd
    SET items = COALESCE(p_items, '[]'::jsonb), updated_at = now()
    WHERE gcd.session_id = v_session_id AND gcd.device_id = p_device_id;
  END IF;

  UPDATE public.table_sessions
  SET guest_count = (SELECT COUNT(*) FROM public.guest_cart_drafts gcd WHERE gcd.session_id = v_session_id)
